...
# Creation Date: segunda-feira, 10 dezembro 2018 07:30
...
# Já não exitem mais.
#
# __init__ agora requer quatro keyword arguments, todos default.
# __init__ agora cria automaticamente a pasta para as imagens.
# createImgFolder() agora não requer mais nenhum parametro.
# createImgFolder() agora usa self.folder como input e retorna
# o caminho da pasta para as imagens.
# createImgFolder() não consegue mais, como input, um caminho especifico.
...
class DownloaderImg:
    ...
    # Multiprocessing...
    def __init__(self, url,*,folder="img", path=None, mode="NORMAL", home=False):
        self.url = url
        self.folder = folder
        self.mode = mode
        self.home = home
        if path is None:
            # Caminho para a pasta das img
            self.path = self.createImgFolder()
        else:
            self.path = self.createImgFolder(self.folder)
        self.parsed_links = []
        self.url_success = []
    ...
    def createImgFolder(self):
        ...
        #
        # Caso o caminho for específico
        # não usar \ (backslash) e sim / (forward slash) por enquanto.
        img_folder = tools.join_paths(self.folder) # Custom path/join converter
